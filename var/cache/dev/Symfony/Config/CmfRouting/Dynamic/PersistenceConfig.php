...
class PersistenceConfig 
{
     ...
     * @psalm-return (TValue is array ? \Symfony\Config\CmfRouting\Dynamic\Persistence\PhpcrConfig : static)
     */
    public function phpcr(array|bool $value = []): \Symfony\Config\CmfRouting\Dynamic\Persistence\PhpcrConfig|static
    {
        if (!\is_array($value)) {
     ...
     * @psalm-return (TValue is array ? \Symfony\Config\CmfRouting\Dynamic\Persistence\OrmConfig : static)
     */
    public function orm(array|bool $value = []): \Symfony\Config\CmfRouting\Dynamic\Persistence\OrmConfig|static
    {
        if (!\is_array($value)) {
    ...
    {
        if (array_key_exists('phpcr', $config)) {
            $this->_usedProperties['phpcr'] = true;
            $this->phpcr = \is_array($config['phpcr']) ? new \Symfony\Config\CmfRouting\Dynamic\Persistence\PhpcrConfig($config['phpcr']) : $config['phpcr'];
            unset($config['phpcr']);
        }

        if (array_key_exists('orm', $config)) {
            $this->_usedProperties['orm'] = true;
            $this->orm = \is_array($config['orm']) ? new \Symfony\Config\CmfRouting\Dynamic\Persistence\OrmConfig($config['orm']) : $config['orm'];
            unset($config['orm']);
        }
