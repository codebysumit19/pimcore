...
class DataObjectsConfig 
{
    ...
    public function __construct(array $config = [])
    {
        if (array_key_exists('classes', $config)) {
            $this->_usedProperties['classes'] = true;
            $this->classes = array_map(fn ($v) => new \Symfony\Config\PimcoreAdmin\GdprDataExtractor\DataObjects\ClassesConfig($v), $config['classes']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['classes'])) {
            $output['classes'] = array_map(fn ($v) => $v->toArray(), $this->classes);
        }
