...
class GdprDataExtractorConfig 
{
    ...
    {
        if (array_key_exists('dataObjects', $config)) {
            $this->_usedProperties['dataObjects'] = true;
            $this->dataObjects = new \Symfony\Config\PimcoreAdmin\GdprDataExtractor\DataObjectsConfig($config['dataObjects']);
            unset($config['dataObjects']);
        }

        if (array_key_exists('assets', $config)) {
            $this->_usedProperties['assets'] = true;
            $this->assets = new \Symfony\Config\PimcoreAdmin\GdprDataExtractor\AssetsConfig($config['assets']);
            unset($config['assets']);
        }
