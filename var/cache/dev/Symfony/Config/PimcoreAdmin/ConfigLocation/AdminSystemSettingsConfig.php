...
class AdminSystemSettingsConfig 
{
    ...
    /**
     * @default {"type":"symfony-config","options":{"directory":"\/var\/www\/my-project\/var\/config\/admin_system_settings"}}
     */
    ...
    {
        if (array_key_exists('write_target', $config)) {
            $this->_usedProperties['writeTarget'] = true;
            $this->writeTarget = new \Symfony\Config\PimcoreAdmin\ConfigLocation\AdminSystemSettings\WriteTargetConfig($config['write_target']);
            unset($config['write_target']);
        }

        if (array_key_exists('read_target', $config)) {
            $this->_usedProperties['readTarget'] = true;
            $this->readTarget = new \Symfony\Config\PimcoreAdmin\ConfigLocation\AdminSystemSettings\ReadTargetConfig($config['read_target']);
            unset($config['read_target']);
        }
