...
class AssetsConfig 
{
    ...
    public function __construct(array $config = [])
    {
        if (array_key_exists('notes_events', $config)) {
            $this->_usedProperties['notesEvents'] = true;
            $this->notesEvents = new \Symfony\Config\PimcoreAdmin\Assets\NotesEventsConfig($config['notes_events']);
            unset($config['notes_events']);
        }

        if (array_key_exists('hide_edit_image', $config)) {
            $this->_usedProperties['hideEditImage'] = true;
            $this->hideEditImage = $config['hide_edit_image'];
            unset($config['hide_edit_image']);
        }

        if (array_key_exists('disable_tree_preview', $config)) {
            $this->_usedProperties['disableTreePreview'] = true;
            $this->disableTreePreview = $config['disable_tree_preview'];
            unset($config['disable_tree_preview']);
        }
