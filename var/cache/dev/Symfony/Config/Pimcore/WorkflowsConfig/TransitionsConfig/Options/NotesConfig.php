...
class NotesConfig 
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (array_key_exists('additionalFields', $config)) {
            $this->_usedProperties['additionalFields'] = true;
            $this->additionalFields = array_map(fn ($v) => new \Symfony\Config\Pimcore\WorkflowsConfig\TransitionsConfig\Options\Notes\AdditionalFieldsConfig($v), $config['additionalFields']);
            unset($config['additionalFields']);
        }

        if (array_key_exists('customHtml', $config)) {
            $this->_usedProperties['customHtml'] = true;
            $this->customHtml = new \Symfony\Config\Pimcore\WorkflowsConfig\TransitionsConfig\Options\Notes\CustomHtmlConfig($config['customHtml']);
