...
class OptionsConfig 
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (array_key_exists('notes', $config)) {
            $this->_usedProperties['notes'] = true;
            $this->notes = new \Symfony\Config\Pimcore\WorkflowsConfig\TransitionsConfig\Options\NotesConfig($config['notes']);
            unset($config['notes']);
        }
        ...
        if (array_key_exists('notificationSettings', $config)) {
            $this->_usedProperties['notificationSettings'] = true;
            $this->notificationSettings = array_map(fn ($v) => new \Symfony\Config\Pimcore\WorkflowsConfig\TransitionsConfig\Options\NotificationSettingsConfig($v), $config['notificationSettings']);
            unset($config['notificationSettings']);
        }

        if (array_key_exists('changePublishedState', $config)) {
            $this->_usedProperties['changePublishedState'] = true;
            $this->changePublishedState = $config['changePublishedState'];
            unset($config['changePublishedState']);
        }

        if (array_key_exists('unsavedChangesBehaviour', $config)) {
            $this->_usedProperties['unsavedChangesBehaviour'] = true;
            $this->unsavedChangesBehaviour = $config['unsavedChangesBehaviour'];
            unset($config['unsavedChangesBehaviour']);
        }
