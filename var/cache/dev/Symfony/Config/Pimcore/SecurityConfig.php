...
class SecurityConfig 
{
    ...
    public function __construct(array $config = [])
    {
        if (array_key_exists('password', $config)) {
            $this->_usedProperties['password'] = true;
            $this->password = new \Symfony\Config\Pimcore\Security\PasswordConfig($config['password']);
            unset($config['password']);
        }

        if (array_key_exists('factory_type', $config)) {
            $this->_usedProperties['factoryType'] = true;
            $this->factoryType = $config['factory_type'];
            unset($config['factory_type']);
        }

        if (array_key_exists('encoder_factories', $config)) {
            $this->_usedProperties['encoderFactories'] = true;
            $this->encoderFactories = array_map(fn ($v) => \is_array($v) ? new \Symfony\Config\Pimcore\Security\EncoderFactoriesConfig($v) : $v, $config['encoder_factories']);
            unset($config['encoder_factories']);
        }

        if (array_key_exists('password_hasher_factories', $config)) {
            $this->_usedProperties['passwordHasherFactories'] = true;
            $this->passwordHasherFactories = array_map(fn ($v) => \is_array($v) ? new \Symfony\Config\Pimcore\Security\PasswordHasherFactoriesConfig($v) : $v, $config['password_hasher_factories']);
            unset($config['password_hasher_factories']);
        }
