...
class ClassDefinitionsConfig 
{
    ...
    public function __construct(array $config = [])
    {
        if (array_key_exists('data', $config)) {
            $this->_usedProperties['data'] = true;
            $this->data = new \Symfony\Config\Pimcore\Objects\ClassDefinitions\DataConfig($config['data']);
            unset($config['data']);
        }

        if (array_key_exists('layout', $config)) {
            $this->_usedProperties['layout'] = true;
            $this->layout = new \Symfony\Config\Pimcore\Objects\ClassDefinitions\LayoutConfig($config['layout']);
            unset($config['layout']);
        }
