...
class ObjectsConfig 
{
    ...
    {
        if (array_key_exists('ignore_localized_query_fallback', $config)) {
            $this->_usedProperties['ignoreLocalizedQueryFallback'] = true;
            $this->ignoreLocalizedQueryFallback = $config['ignore_localized_query_fallback'];
            unset($config['ignore_localized_query_fallback']);
        }
        ...
        if (array_key_exists('versions', $config)) {
            $this->_usedProperties['versions'] = true;
            $this->versions = new \Symfony\Config\Pimcore\Objects\VersionsConfig($config['versions']);
            unset($config['versions']);
        }

        if (array_key_exists('custom_layouts', $config)) {
            $this->_usedProperties['customLayouts'] = true;
            $this->customLayouts = new \Symfony\Config\Pimcore\Objects\CustomLayoutsConfig($config['custom_layouts']);
            unset($config['custom_layouts']);
        }

        if (array_key_exists('select_options', $config)) {
            $this->_usedProperties['selectOptions'] = true;
            $this->selectOptions = new \Symfony\Config\Pimcore\Objects\SelectOptionsConfig($config['select_options']);
            unset($config['select_options']);
        }

        if (array_key_exists('class_definitions', $config)) {
            $this->_usedProperties['classDefinitions'] = true;
            $this->classDefinitions = new \Symfony\Config\Pimcore\Objects\ClassDefinitionsConfig($config['class_definitions']);
