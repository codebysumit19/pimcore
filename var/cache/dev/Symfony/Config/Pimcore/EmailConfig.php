...
class EmailConfig 
{
    ...
    public function __construct(array $config = [])
    {
        if (array_key_exists('sender', $config)) {
            $this->_usedProperties['sender'] = true;
            $this->sender = new \Symfony\Config\Pimcore\Email\SenderConfig($config['sender']);
            unset($config['sender']);
        }

        if (array_key_exists('return', $config)) {
            $this->_usedProperties['return'] = true;
            $this->return = new \Symfony\Config\Pimcore\Email\ReturnConfig($config['return']);
            unset($config['return']);
        }

        if (array_key_exists('debug', $config)) {
            $this->_usedProperties['debug'] = true;
            $this->debug = new \Symfony\Config\Pimcore\Email\DebugConfig($config['debug']);
            unset($config['debug']);
        }

        if (array_key_exists('usespecific', $config)) {
            $this->_usedProperties['usespecific'] = true;
            $this->usespecific = $config['usespecific'];
            unset($config['usespecific']);
        }
