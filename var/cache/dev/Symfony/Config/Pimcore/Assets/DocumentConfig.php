...
class DocumentConfig 
{
    ...
    public function __construct(array $config = [])
    {
        if (array_key_exists('thumbnails', $config)) {
            $this->_usedProperties['thumbnails'] = true;
            $this->thumbnails = new \Symfony\Config\Pimcore\Assets\Document\ThumbnailsConfig($config['thumbnails']);
            unset($config['thumbnails']);
        }

        if (array_key_exists('process_page_count', $config)) {
            $this->_usedProperties['processPageCount'] = true;
            $this->processPageCount = $config['process_page_count'];
            unset($config['process_page_count']);
        }

        if (array_key_exists('process_text', $config)) {
            $this->_usedProperties['processText'] = true;
            $this->processText = $config['process_text'];
            unset($config['process_text']);
        }

        if (array_key_exists('scan_pdf', $config)) {
            $this->_usedProperties['scanPdf'] = true;
            $this->scanPdf = $config['scan_pdf'];
            unset($config['scan_pdf']);
        }

        if (array_key_exists('open_pdf_in_new_tab', $config)) {
            $this->_usedProperties['openPdfInNewTab'] = true;
            $this->openPdfInNewTab = $config['open_pdf_in_new_tab'];
            unset($config['open_pdf_in_new_tab']);
        }
