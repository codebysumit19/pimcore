...
class ThumbnailsConfig 
{
    ...
    {
        if (array_key_exists('allowed_formats', $config)) {
            $this->_usedProperties['allowedFormats'] = true;
            $this->allowedFormats = $config['allowed_formats'];
            unset($config['allowed_formats']);
        }

        if (array_key_exists('max_scaling_factor', $config)) {
            $this->_usedProperties['maxScalingFactor'] = true;
            $this->maxScalingFactor = $config['max_scaling_factor'];
            unset($config['max_scaling_factor']);
        }
