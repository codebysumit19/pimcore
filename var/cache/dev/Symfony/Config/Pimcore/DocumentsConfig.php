...
class DocumentsConfig 
{
    ...
    {
        if (array_key_exists('doc_types', $config)) {
            $this->_usedProperties['docTypes'] = true;
            $this->docTypes = new \Symfony\Config\Pimcore\Documents\DocTypesConfig($config['doc_types']);
            unset($config['doc_types']);
        }

        if (array_key_exists('versions', $config)) {
            $this->_usedProperties['versions'] = true;
            $this->versions = new \Symfony\Config\Pimcore\Documents\VersionsConfig($config['versions']);
        ...
        if (array_key_exists('error_pages', $config)) {
            $this->_usedProperties['errorPages'] = true;
            $this->errorPages = new \Symfony\Config\Pimcore\Documents\ErrorPagesConfig($config['error_pages']);
            unset($config['error_pages']);
        }

        if (array_key_exists('allow_trailing_slash', $config)) {
            $this->_usedProperties['allowTrailingSlash'] = true;
            $this->allowTrailingSlash = $config['allow_trailing_slash'];
            unset($config['allow_trailing_slash']);
        }

        if (array_key_exists('generate_preview', $config)) {
            $this->_usedProperties['generatePreview'] = true;
            $this->generatePreview = $config['generate_preview'];
            unset($config['generate_preview']);
        }

        if (array_key_exists('preview_url_prefix', $config)) {
            $this->_usedProperties['previewUrlPrefix'] = true;
            $this->previewUrlPrefix = $config['preview_url_prefix'];
            unset($config['preview_url_prefix']);
        }
        ...
        if (array_key_exists('editables', $config)) {
            $this->_usedProperties['editables'] = true;
            $this->editables = new \Symfony\Config\Pimcore\Documents\EditablesConfig($config['editables']);
            unset($config['editables']);
        }

        if (array_key_exists('areas', $config)) {
            $this->_usedProperties['areas'] = true;
            $this->areas = new \Symfony\Config\Pimcore\Documents\AreasConfig($config['areas']);
            unset($config['areas']);
        }

        if (array_key_exists('auto_save_interval', $config)) {
            $this->_usedProperties['autoSaveInterval'] = true;
            $this->autoSaveInterval = $config['auto_save_interval'];
            unset($config['auto_save_interval']);
        }

        if (array_key_exists('static_page_router', $config)) {
            $this->_usedProperties['staticPageRouter'] = true;
            $this->staticPageRouter = new \Symfony\Config\Pimcore\Documents\StaticPageRouterConfig($config['static_page_router']);
            unset($config['static_page_router']);
        }

        if (array_key_exists('static_page_generator', $config)) {
            $this->_usedProperties['staticPageGenerator'] = true;
            $this->staticPageGenerator = new \Symfony\Config\Pimcore\Documents\StaticPageGeneratorConfig($config['static_page_generator']);
            unset($config['static_page_generator']);
        }

        if (array_key_exists('type_definitions', $config)) {
            $this->_usedProperties['typeDefinitions'] = true;
            $this->typeDefinitions = new \Symfony\Config\Pimcore\Documents\TypeDefinitionsConfig($config['type_definitions']);
