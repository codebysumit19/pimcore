...
class HousekeepingConfig 
{
     ...
     * @return $this
     */
    public function cleanupTmpFilesAtimeOlderThan($value): static
    {
     ...
     * @return $this
     */
    public function cleanupProfilerFilesAtimeOlderThan($value): static
    {
    ...
    {
        if (array_key_exists('cleanup_tmp_files_atime_older_than', $config)) {
            $this->_usedProperties['cleanupTmpFilesAtimeOlderThan'] = true;
            $this->cleanupTmpFilesAtimeOlderThan = $config['cleanup_tmp_files_atime_older_than'];
            unset($config['cleanup_tmp_files_atime_older_than']);
        }

        if (array_key_exists('cleanup_profiler_files_atime_older_than', $config)) {
            $this->_usedProperties['cleanupProfilerFilesAtimeOlderThan'] = true;
            $this->cleanupProfilerFilesAtimeOlderThan = $config['cleanup_profiler_files_atime_older_than'];
            unset($config['cleanup_profiler_files_atime_older_than']);
        }
